...
parser.add_argument('--DIM_CON', type=float, default=150, help='l, the dimensionality constraint for displacement vectors')
...
class ComputeOT:
    ...
    def __init__(self, sess, input_dim, f_model, g_model, lr, sa_it, sa_mt, sa_dr):
        ...
        self.GT = np.load(os.path.join(data_folder_sicnn, 'high_t.npy'))
        ...
        # smoothed L0 norm
        epsilon = 1  # should based on the length of displacement vector
    ...
    def learn(self, batch_size, iters, inner_loop_iterations, scale, variance, dataset_x, dataset_y, plot_size, experiment_name, opt, initial_lambda, dim_con):
    
        print_T = 100 # Gap of iteration to print training statistics
        restricted_dim = dim_con
        cnt = 0

        # Learning and smoothing and initiation hyper-parameters, could be adjusted based the size of user's dataset
        initiation = 10000
        adjust_iters = 5000
        tf.compat.v1.disable_eager_execution()
        ...
        while True:
            ...
            # Increase lambda, could be switched to the heuristic adjustment in Appendix B, Algorithm 2;
            # In practice, we find both method could work, while the constant increasing rate is more straightforward
            current_lambda *= 1.5
        ...
        for iteration in range(initial_iters, iters):
            ...
            if iteration == initial_iters or (iteration > initial_iters and (iteration - initial_iters) % adjust_iters == 0):
                ...
                # Decay the temperature, also could be switched to heuristic decreasing method
...
def sample_data_gen(DATASET, BATCH_SIZE, SCALE , VARIANCE):
    ...
    if DATASET == 't':
        while True:
            dataset_Y = np.load(os.path.join(data_folder_sicnn, 'high_t.npy'))
            yield dataset_Y
    
    elif DATASET == 'c':
        while True:
            dataset_Y = np.load(os.path.join(data_folder_sicnn, 'high_c.npy'))
            yield dataset_Y 
